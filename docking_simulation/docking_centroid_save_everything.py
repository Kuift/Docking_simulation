...
def dock(out: Path = None, centroid: Optional[str] = None, box: Optional[str] = None, exhaustiveness: int = 30, num_modes: int = 25):
    ...
    CsAP2L1_centroid.append(np.array([  -3.142,   6.795,  -4.064]))
    ...
    for structure in structures:
        ...
        for j, cent in enumerate(centroids[structure_name]):
            for substrate in substrates:
                for i in range(150):
                    os.mkdir(f'results/{str(structure_name)}-{str(substrate)}-{str(exhaustiveness)}-{str(num_modes)}-Centroid_INDEX{j}-run{i}')
                    ...
                    poses, scores = vpg.generate_poses(
                        ...
                        num_modes=num_modes,
                        out_dir=f'results/{str(structure_name)}-{str(substrate)}-{str(exhaustiveness)}-{str(num_modes)}-Centroid_INDEX{j}-run{i}',
